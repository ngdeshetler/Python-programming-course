...
def get_word_score(word, n):
    ...
    score=0 #assume no score
    if len(word) == n: #if word is the length of n add bonus
    	bonus=50
    for i in word:
    	score += SCRABBLE_LETTER_VALUES[i] #add points for each letter
    return score+bonus
...
def is_valid_word(word, hand, word_list):
    ...
    """
    # TO DO ...
    is_word = word in word_list #a boolean that indicates that word is a valid word from word_list
    is_hand = True #start off assuming its in the hand
    ...
    for i in freq: #looks at each letter in word
    	...
    	#than the frequency in the hand its not possible in the hand, so we set to false
    		is_hand = False
    return is_word & is_hand #only returns True when both are true
    ...
    * The final score is displayed.
    ...
    while not dead_hand:
    	...
    	if word == '.':
    		...
    		running_score += get_word_score(word, get_n(hand)) #adds word score to running score
    		print '\n\t"' + word + '"' + ' earned ' + str(get_word_score(word, get_n(hand))) + ' points.'
    		current_hand=update_hand(current_hand, word) #update hand
    		if get_n(current_hand) > 0: #if still letters left in hand
...
def get_n(hand):
	"""
	I added this function to give me the total count of possible letters in a hand.
